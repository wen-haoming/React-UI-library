 ...
 *  头部控制的hooks
 */
export function useHandleHeader(yearNum: number = 1, monthNum: number = 1) {
  const {
    year,
    month,
    changeDate,
  ...
  return useMemo(() => {
    return {
      rrBtn: () => {
        changeDate(year + yearNum, false, false);
      },
      llBtn: () => {
        changeDate(year - yearNum, false, false);
      },
      rBtn: () => {
        if (month === 12) {
          ...
          changeDate(false, month + monthNum, false);
        }
      },
      lBtn: () => {
        if (month === 1) {
...
export function useRenderDate() {
  ...
  // 获取下个月的天数进行尾部填补
  const { year, month, format } = useContext<ContextType>(Context);
...
export function useRenderMonth() {
  const { year } = useContext<ContextType>(Context);
